...
datavec & sum(datavec & lhs, datavec const & rhs) { // если можно менять старую память, экономим на allocation
...
datavec & sum(datavec & lhs, datavec & rhs) { // если можно менять старую память, экономим на allocation
	if (lhs.size() < rhs.size()) {
		swap(lhs, rhs);
	}
	xint carry = 0;
...
datavec multiply_switch(datavec const & lhs, datavec const & rhs) { // выбирает метод и исполняет
	datavec res;
